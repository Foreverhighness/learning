...
void eval(char *cmdline) {
    ...
    if (!builtin_cmd(argv)) {
        sigprocmask(SIG_BLOCK, &mask_chld, &prev_chld);  // Block SIGCHLD
        if ((pid = fork()) == 0) {                       // Child
            setpgid(0, 0);
        ...
        if (is_bg) {
            sigprocmask(SIG_BLOCK, &mask_all, NULL);  // race condition
            assert(addjob(jobs, pid, BG, cmdline));
            printf("[%d] (%d) %s", pid2jid(pid), pid, cmdline);
            sigprocmask(SIG_SETMASK, &prev_chld, NULL);
        } else {
            sigprocmask(SIG_BLOCK, &mask_all, NULL);  // race condition
            assert(addjob(jobs, pid, FG, cmdline));
            sigprocmask(SIG_SETMASK, &prev_chld, NULL);
            waitfg(pid);
        }
    }
 ...
 */
void do_bgfg(char **argv) {
    assert(argv != NULL && argv[0] != NULL);  // check argument
    assert(fgpid(jobs) == 0);                 // should no fg process
    struct job_t *job;
    ...
    assert(job->state != FG && job->state != UNDEF);
    if (job->state == ST) {
        kill(-pid, SIGCONT);
    }
    if (bg) {
