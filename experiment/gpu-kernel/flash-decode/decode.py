...
def decode_map(
    ...
    for n in range(0, seq_len, B_col):
        ...
        # score_ij = score_ij * scale # scale ops: seq_len / B_col * B_col
...
def decode_reduce(
    ...
    for id_split in range(0, NUM_SPLITS):
        ...
        if split_start <= split_end:
            midO_i = tl.load(MidO + offs_mido + id_split * stride_midOd)
            logsumexp_i = tl.load(Logsumexp + id_split * stride_Ld)
...
def flash_decode(Q: torch.Tensor, K: torch.Tensor, V: torch.Tensor) -> torch.Tensor:
    ...
    grid = (NUM_SPLITS, batch_size * num_heads, 1)
    ...
    print(stride_q, stride_k, stride_v, stride_midO)
    decode_map[grid](
